...
module.exports.insertAdministrator=async (number,name,password)=>{
  const query_sql='insert into chendr21_Administrator(administrator_number,administrator_name,administrator_password) values(@number,@name,@password)'
  return await this.query_safe(query_sql,{number,name,password})
}

module.exports.selectAdministrator=async (number,password)=>{
  const query_sql='select * from chendr21_Administrator where administrator_number=@number and administrator_password=@password'
  return await this.query_safe(query_sql,{number,password})
}

module.exports.selectCollegeManager=async(number,password)=>{
  const query_sql='select * from shenwq21_College_Manager where college_manager_number=@number and college_manager_password=@password'
  return await this.query_safe(query_sql,{number,password})
}