...
module.exports.postStudent=async (ctx,next)=>{
  ...
  if(!isLimited(payload.role,['administrator'])){
    ...
    const {name,number,password,class_id,sex='',birth='',birth_place='',tel=''}=ctx.request.body
    if(!name || !number || !password || !class_id){
      ...
      if(exist.recordset.length>0){
        ...
        const result=await model.insertStudent(name,number,md5(password),class_id,sex,birth,birth_place,tel)
        ctx.body={
          code:0,
          msg:'insert'
        }
